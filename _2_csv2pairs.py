...
def checkinList(term, term_list):
  for cterm in term_list:
    if cterm.replace(" ","-") == term.replace(' ','-'):
      return True
  return False
...
def convert2pairs(filename, bias_spec):
  ...
  for bias in bias_spec:
    ...
    for idx, row in gen_df[gen_df['Bias Name'] == name].iterrows():
      ...
      pairs.append([name, row['Attribute word'], row['Template version'].replace("[T]","[MASK]"), grp_term_pair[0], grp_term_pair[1], direction[0], direction[1], row['Discarded'], row['Reason for discard']])
