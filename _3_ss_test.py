...
# add period to end the sentences if no punctuation
def add_period(template):
  if template[-1] != string.punctuation:
      template += "."
  return template
...
def getBERTProb(model, tokenizer, template, targets, device, verbose=False):
  ...
  for target in targets:
    ...
    logits = prior_logits[0][0][get_mask_idx(prior_token_ids)][targ_id]
    if verbose:
      print("Logits:", logits)
...
def checkBias(row, biasProbFunc, model, tokenizer, device):
  ...
  test_res, sentences = biasProbFunc(model, tokenizer, add_period(row['template']), att_terms, device)
...
def testBiasOnPairs(source_path, dest_path, model_name, model, tokenizer, device):
  ...
  for gen_file in gen_files:
    ...
    dest_model_path = os.path.join(dest_path, model_name)
